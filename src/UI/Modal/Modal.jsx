...
export default function Modal({componentId, setModalId}) {
	...
	const closeModal = useCallback(() => { //функция для закрытия модального окна путем смены состояния и обнулени id компонента
		setModalId('');
	}, [setModalId])

	useEffect(() => { // слушатель для закрытия окна по esc
	...
	if ( componentId === '') { //проверка отрисовки модалки в DOM
		return null;
	}
		...
		createPortal(
			<div className={style.modal} onClick={closeModal}>
				<div className={style.modal__content} onClick={(e) => {e.stopPropagation()}}>
					<button type="button" onClick={closeModal} className={style.modal__close}>
						<svg
							width="12" 	
							height="12"
							viewBox="0 0 12 12"
